...
const MainLandingScreen = () => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const fetchUserData = async () => {
        ...
        // Assume fetching other data remains the same
        ...
        if (token === 'test') {
          ...
          setUserData({
            nickname: name,
            age,
            height: height || userData.height, // Use fetched height or existing height
            weight: weight || userData.weight, // Use fetched weight or existing weight
          });
        }
      };
      fetchUserData();
  ...
  const logout = async () => {
    ...
    await AsyncStorage.removeItem('@bloodHistory');
    ...
    <>
      <ScrollView
        ...
        <View style={styles.chartCard}>
          <Text style={styles.sectionTitle}>혈액분석 대시보드</Text>
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          ...
          <Text style={styles.iconText}>혈액검사</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => navigation.navigate('UserProfileUpdate')}
          style={styles.iconButton}>
          <Icon name="cog" size={20} color="#3C5A99" />
